...
#Plot 3
df %>% select(PERCENT_FINLOAN, STATE, AVGGRADDEBT) %>% filter(AVGGRADDEBT != "null", PERCENT_FINLOAN != "null") %>% mutate(state_name = state.name[match(STATE, state.abb)]) %>% group_by(state_name) %>% summarize(state_avg_loan = mean(as.numeric(as.character(PERCENT_FINLOAN))), state_avg_debt = mean(as.numeric(as.character (AVGGRADDEBT)))) %>% ggplot(aes(x = reorder(state_name, state_avg_debt), y = state_avg_debt, fill = state_name)) + labs(title = "University Debt by State", x = "Average Debt", y = "State") + theme(legend.position = "none") + geom_bar(stat = "identity") + coord_flip()
